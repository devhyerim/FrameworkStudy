...
<script type="text/javascript">
	...
	{
		// Ajax 요청 및 응답 처리
		// 직위 선택 내용이 변경되었을 때 변경된 기본급이 보여지도록.
		ajaxRequest();
		
		
		// datePicker가 운용되는 곳 → 생년월일(id: birthday)
		// jQuery-UI 캘린더를 불러오는 함수처리 (datepicker())
		$("#birthday").datepicker(
		{
			dateFormat: "yy-mm-dd"
			, changeMonth: true
		...
		$("#submitBtn").click(function()
		{
			...
			//-- 라디오박스, 셀렉트 박스는 이미 선택되어 있으므로 공란 여부 파악X
			
			// 최소 기본급에 대한 유효성 검사
			// alert($("#minBasicPay").val()); //--(X)
			// ※ val() 은 아무값도 나오지 않는다.
			
			// alert($("#minBasicPay").text()); --> 1000000
			
			// basicPay는 textbox 로 사용자에게 입력받음 → val() 로 가져올 수 있고,
			// minBasicPay는 AJAX 처리로 html을 통해 data를 입력 받은 것 → val()XXX text()로 가져올 수 있다.
	...
	{
		// '$.post()' / '$.get()'
		//-- jQuery에서 AJAX를 사용해야 할 경우 지원해주는 함수
		...
		<form action="employeeupdate.action" method="post" id="employeeForm">
			<table>
				...
				<tr>
					<th>양/음력</th>
					<td>
						<input type="radio" value="0" id="lunar0" name="lunar"
					...
					<td>
						<select name="regionId" id="regionId">
							...
							<c:forEach var="region" items="${regionList }">
								...
								"selected=\"selected\"" : "" }>${region.regionName }
								</option>
							</c:forEach>
						</select>
					</td>
					...
					<td>
						<select name="positionId" id="positionId">
							...
							<c:forEach var="position" items="${positionList }">
								...
								"selected=\"selected\"" : "" }>${position.positionName }
								</option>
							</c:forEach>
						</select>
					</td>
